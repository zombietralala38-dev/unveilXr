...
// --- Generación de nombres aleatorios (sin IL_POOL ni HANDLER_POOL) ---
function randomName() {
...
// --- Ofuscación numérica pesada (sin cambios) ---
...
// --- VM basada SOLO EN XOR (ahora con bit32.bxor) ---
function buildTrueVM(payloadStr) {
  ...
  for(let i = 0; i < totalChunks; i++) {
    ...
    if (currentReal < realChunks.length && (Math.random() > 0.5 || (totalChunks - i) === (realChunks.length - currentReal))) {
      ...
      for(let j = 0; j < chunk.length; j++) { 
        // Cifrado XOR simple: byte ^ (seed + índice)
  ...
  // Descifrado con bit32.bxor (compatible con Roblox)
  vmCore += `local _gIdx=0 for _, ${idxVar} in ipairs(${ORDER}) do for _, ${byteVar} in ipairs(_pool[${idxVar}]) do `
  vmCore += `if type(math.pi)=="string" then ${KEY}=${KEY}+1 end `
...
module.exports = { obfuscate }const HEADER = `--[[ this code it's protected by vvmer obfuscator ]]`
