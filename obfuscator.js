// ============================================================
//  Seak Obfuscator v6.0 - Luraph-style VM + XOR dominance
// ============================================================
...
const CONFIG = {
    ...
    antiEnv: true,
    vmCapas: 15,
    junkLines: 60,          // basura reducida para no saturar
    xorIntensity: 0.7       // 70% de las ofuscaciones usan XOR, 30% heavyMath
};
...
// ---------- NÚMEROS CON XOR (SIMPLES) ----------
function xorNumber(n) {
    const key = Math.floor(Math.random() * 200) + 30;
    return `bit32.bxor(${n}, ${key})`;  // se deshace aplicando XOR con la misma key
}

// ---------- HEAVY MATH (USO REDUCIDO) ----------
function heavyMath(n) {
    if (Math.random() < 0.3) {  // solo 30% de heavyMath cuando se requiera
    ...
    return n.toString();  // el resto se queda como número literal
}

// ---------- OFUSCAR UN NÚMERO (ELECCIÓN ALEATORIA) ----------
function obfNumber(n) {
    if (Math.random() < CONFIG.xorIntensity) {
        return xorNumber(n);
    } else {
        return heavyMath(n);
    }
...
// ---------- CIFRADO DE STRING CON XOR (RETORNA ARRAY DE BYTES OFUSCADOS) ----------
function xorStringToBytes(str) {
    const key = Math.floor(Math.random() * 200) + 30;
    const bytes = [];
    for (let i = 0; i < str.length; i++) {
        const encrypted = str.charCodeAt(i) ^ key;
        bytes.push(obfNumber(encrypted));
    }
    return { bytes, key };
}

// ---------- GENERADOR DE BASURA (MENOS VARIABLES LOCALES) ----------
...
// ---------- VM ESTILO LURAPH (INSTRUCCIONES CIFRADAS CON XOR) ----------
function buildLuraphVM(code) {
    ...
    // Crear tabla de "instrucciones" (cada instrucción es un chunk cifrado con XOR)
    const instrTable = randomName();
    let init = `local ${instrTable} = {};\n`;
    chunks.forEach((chunk, idx) => {
        const encryptedBytes = chunk.split('').map(c => c.charCodeAt(0) ^ key);
        init += `${instrTable}[${obfNumber(idx + 1)}] = {${encryptedBytes.map(b => obfNumber(b)).join(',')}};\n`;
    });

    // Decodificador: recorre las instrucciones, aplica XOR y junta el string
...
// ---------- CAPA DE DISPATCHER (SIMILAR A LURAPH) ----------
function buildDispatcher(innerCode) {
    const handlers = [];
    const numHandlers = 2 + Math.floor(Math.random() * 3);
    const realIdx = Math.floor(Math.random() * numHandlers);
...
// ---------- ANTI-ENV LOGGER (CAMUFLADO CON XOR) ----------
function buildAntiEnv() {
    ...
    chunks.forEach(chunk => {
        ...
        // Almacenar clave y bytes ofuscados como subtabla
        lines.push(`table.insert(${tableName}, {${obfNumber(key)}, {${bytes.join(',')}}});`);
    });

    // Reconstructor con XOR
    const reconstName = randomName();
    lines.push(`
        local ${reconstName} = "";
        for _, __entry in ipairs(${tableName}) do
            ...
            for _, __b in ipairs(__bytes) do
                ${reconstName} = ${reconstName} .. string.char(bit32.bxor(__b, __key));
            end
        end
        assert(loadstring(${reconstName}))();
    `);
...
function obfuscate(sourceCode) {
    ...
    for (let i = 0; i < CONFIG.junkLines; i++) {
        junkLines.push(generateJunkLine());
    }

    // Mezclar anti-env en posiciones aleatorias (evitando el principio)
    const mixed = [...junkLines];
    antiEnvLines.forEach(line => {
        const pos = Math.floor(Math.random() * (mixed.length - 3)) + 2;
