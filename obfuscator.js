...
// --- Aritmética pesada para ocultar números (sin cambios) ---
...
// Junk generado sin usar IL_POOL (todos los nombres son aleatorios)
...
// --- VM basada SOLO EN XOR (cifrado y descifrado) ---
function buildTrueVM(payloadStr) {
  const STACK = randomName()
  const KEY = randomName()
  const ORDER = randomName()
  
  const seed = Math.floor(Math.random() * 200) + 50   // clave base
  ...
  // Decodificación XOR pura
  vmCore += `local _gIdx=0 for _, ${idxVar} in ipairs(${ORDER}) do for _, ${byteVar} in ipairs(_pool[${idxVar}]) do `
  vmCore += `if type(math.pi)=="string" then ${KEY}=${KEY}+1 end `  // corruptor silencioso simple
  vmCore += `table.insert(${STACK}, string.char(${byteVar} ~ ((${KEY} + _gIdx) % 256))) _gIdx=_gIdx+1 end end `
...
// Encapsulamiento en 18 capas de VMs falsas (con CFF y junk)
...
// Protecciones extra (anti‑debug + tamper checks) con nombres aleatorios
