...
// ----------------------------------------------------------------------
// Fragmenta el código en 'numParts' partes (por defecto 2883)
// ----------------------------------------------------------------------
function fragmentCode(code, numParts = 2883) {
  ...
  for (let i = 0; i < numParts; i++) {
    ...
    if (start >= len) break;
    const end = Math.min(start + partSize, len);
    fragments.push(code.substring(start, end));
  }
  // Si por alguna razón salen menos de numParts, rellenamos con strings vacíos
  while (fragments.length < numParts) fragments.push("");
  return fragments;
}

// ----------------------------------------------------------------------
// Función principal de ofuscación
// - sourceCode: el script Lua original
// - returnFragments: si es true, devuelve un array de 2883 fragmentos.
//   Si es false, devuelve el código completo como string.
// ----------------------------------------------------------------------
function obfuscate(sourceCode, returnFragments = true) {
  if (!sourceCode) return returnFragments ? [] : '--ERROR';

  // === INYECCIÓN DE TUS BLOQUES ANTI-ENV-LOGGER ===
  const antiEnvLoggers = `
--[[ ANTI ENV LOGGER BLOQUE 1 ]]
local env = _G
local mt = getmetatable(env)
if mt ~= nil or rawget(env, "__index") ~= nil or rawget(env, "__newindex") ~= nil then
    error("hi detected")
end
local di = debug and debug.getinfo and debug.getinfo(print, "S")
if type(print) ~= "function" or (di and di.what == "C" and di.source ~= "=[C]") then
    error("hi detected")
end
print(" hi pass")

--[[ ANTI ENV LOGGER BLOQUE 2 ]]
...
task.wait(0.07)
print((b.Parent==nil and a.Parent==nil and singularity.p==a and singularity.g==dxfc and singularity.s==0) and 'pass' or 'dtc')
`;

  const fullSource = antiEnvLoggers + "\n" + sourceCode;
  ...
  const match = fullSource.match(isLoadstringRegex);
  if (match) { payloadToProtect = match[1]; } 
  else { payloadToProtect = detectAndApplyMappings(fullSource); }
  ...
  if (returnFragments) {
    return fragmentCode(cleanResult, 2883);
  } else {
    return cleanResult;
  }
}
