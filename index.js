...
const FOOTER_MESSAGE =
  "Thx for use me, im made by 5000 lines of code we only a group of developers. " +
...
async function handleObfuscate(interaction) {
  ...
  if (!check.ok) {
    ...
    return await interaction.editReply({
      embeds: [
        buildErrorEmbed("Invalid input", "I think this is not lua or have some sintaxis error.", [
          { name: "Status", value: "rejected", inline: true },
  ...
  try {
    ...
    return await interaction.editReply({
      embeds: [
        buildErrorEmbed("Obfuscation failed", err.message || String(err), [
  ...
  const fileNameId = Math.floor(10000 + Math.random() * 90000).toString();
  const fileName = `${fileNameId}.txt`;
...
async function handleGet(interaction) {
  ...
  try {
    ...
    return await interaction.editReply({
      embeds: [
        buildErrorEmbed("Fetch failed", err.message || String(err), [
          { name: "Status", value: "failed", inline: true },
          { name: "Time", value: formatDuration(elapsed), inline: true },
        ]),
      ],
...
async function handleSupport(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription("What it's your problem?")
      .setFooter({ text: FOOTER_MESSAGE });
...
async function handleSupportDm(message) {
  ...
  if (session.step === "awaiting_id") {
    ...
    const embed = new EmbedBuilder()
      .setColor(COLOR_BLUE)
      .setTitle("Thanks")
      .setDescription("Wait, some one of sopporters will dm you and sopport.")
      .setFooter({ text: FOOTER_MESSAGE });
    await message.reply({ embeds: [embed] });
